...
$(document).ready(function () {
    ...
    $("#btn_insert").click(function () {
        ...
        OriginArrDataTrade.unshift({
            'id': maxid, 'buysell': orderbuysell, 'done_trade': '0', "detail": {
                "coint_type": coinname,
    ...
    function loaddata(objSortId = 0, objSortName = 0, objSortDate = 0) {
        ...
        $.getJSON("database/data.json", function (data) {
            ...
            $(".tbl_history").append("<tbody>" + result + tmptopbottomrow + "</tbody>");
    ...
    function cloneTblHistory() {
        // remove table from div clone parent
        $("#dv_clone_tbl_history").html("");
        var tmpClone = $('.tbl_history').clone().find('tr+tr').remove().end();
        ...
        // Clone Footer 
        $("#dv_clone_tbl_history_footer").html("");
        tmpClone = $('.tbl_history').clone().find('tr').remove().first();
        $("#dv_clone_tbl_history_footer").html(tmpClone);
    }
    function setVisibiltyOriginTblHistory(obj = 'hidden') {
        $("#dvwrap_tbl_history .tbl_history thead").css("visibility", obj);
